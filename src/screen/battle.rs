...
pub struct Battle {
    ...
    confirmation_receiver_end_turn: Option<Receiver<screen::confirm::Message>>,
}
...
impl Battle {
    pub fn new(
        ...
        Ok(Self {
            ...
            confirmation_receiver_end_turn: None,
        })
    }
    ...
    fn popup_confirm_end_turn(&mut self, context: &mut Context) -> ZResult<Box<dyn Screen>> {
        let (sender, receiver) = channel();
        self.confirmation_receiver_end_turn = Some(receiver);
        let lines = &["Some fighters still can move or attack.", "End turn?"];
        let popup = screen::Confirm::from_lines(context, lines, sender)?;
        Ok(Box::new(popup))
    }
...
impl Screen for Battle {
    fn update(&mut self, context: &mut Context, dtime: Duration) -> ZResult<StackCommand> {
        ...
        if screen::confirm::try_receive_yes(&self.confirmation_receiver_end_turn) {
            self.confirmation_receiver_end_turn = None;
    ...
    fn click(&mut self, context: &mut Context, pos: Point2<f32>) -> ZResult<StackCommand> {
        ...
        match message {
            ...
            Some(Message::EndTurn) => {
                ...
                // TODO: Add an "Options" menu and let the user switch off this dialog.
                if state::has_any_points(&self.state, self.state.player_id()) {
                    let popup = self.popup_confirm_end_turn(context)?;
